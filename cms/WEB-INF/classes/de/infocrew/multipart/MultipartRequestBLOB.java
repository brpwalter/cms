...
public class MultipartRequestBLOB
{
	...
	*/
	 ...
	 * @see #MAX_READ_BYTES
	 */
    public MultipartRequestBLOB(String strContentTypeText, int intContentLength, InputStream in, String strSaveDirectory) throws IllegalArgumentException, IOException
	{
	 ...
	 * @see #MAX_READ_BYTES
	 */
	public MultipartRequestBLOB(String strContentTypeText, int intContentLength, InputStream in, String strSaveDirectory, int intMaxReadBytes) throws IllegalArgumentException, IOException
	{
	 ...
	 * @see #MAX_READ_BYTES
	 */
	public MultipartRequestBLOB(PrintWriter debug, String strContentTypeText, int intContentLength, InputStream in, String strSaveDirectory) throws IllegalArgumentException, IOException
	{
	 ...
	 * @see #MAX_READ_BYTES
	 */
	public MultipartRequestBLOB(PrintWriter debug, String strContentTypeText, int intContentLength, InputStream in, String strSaveDirectory, int intMaxReadBytes) throws IllegalArgumentException, IOException
	{
		this.debug = debug;
	...
	*/
	...
	{
	    try {
		MultipartBlob blob = (MultipartBlob)htFiles.get(strName);
		return blob.contentType;  // may be null
	    }
	    catch (Exception e) {
		return null;
	    }
	...
	*/
	public MultipartBlob getFile(String strName)
	{
	    try {
		MultipartBlob blob = (MultipartBlob)htFiles.get(strName);
		return blob;  // may be null
	    }
	    catch (Exception e) {
		return null;
	    }
	...
	*/
	public String getFileSystemName(String strName)
	{
	    try {
		MultipartBlob blob = (MultipartBlob)htFiles.get(strName);
		return blob.filename;  // may be null
	    }
	    catch (Exception e) {
		return null;
	    }
	...
	*/
	public long getFileSize(String strName)
	{
	    try {
		MultipartBlob blob = (MultipartBlob)htFiles.get(strName);
		return blob.length();  // may be null
	    }
	    catch (Exception e) {
		return (long)-1;
	    }
			...
			else
			{
				...
				else
				{
					if (strFilename.length()>0)
					{
						...
						*/
						ByteArrayOutputStream bstream = readFile (in);
						if (bstream != null) {
						    if (bstream.size () > 0) 
							htFiles.put (strName, new MultipartBlob (bstream.toByteArray (), strFilename, strContentType));
						    else
							htFiles.put (strName, new MultipartBlob (null, null, null));
						    bstream.close ();
						}
	...
	*/
    ...
    private ByteArrayOutputStream readFile (InputStream in) throws IOException {
	ByteArrayOutputStream file = null;
		...
		while(true)
		{
			...
			else
			{
				...
				if (file==null && (endOfLineBytes!=null || length>0))
